    ...
    @endif
        ...
        <form method="POST" action="{{ route('login.sign-in') }}" class="w-[343px] lg:w-[392px]">
            ...
            <div class="flex justify-between items-center mt-[3%] mb-[4%]">
                ...
                <a href="#" class="text-xs-2 text-blue">{{ __('recover_password') }}</a>
            </div>
            <button type="submit" class="btn-primary">{{ __('sign_in') }}</button>

            <p class="text-light-dark text-xs-1 text-center w-[100%] mt-[24px]">{{ __('account') }} <a href="#" class="text-dark text-xs-3">{{ __('signup_link') }}</a></p>
        </form>
